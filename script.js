...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const limitedThreads = ['thread1', 'thread2', 'thread3', 'thread4', 'thread6', 'thread7'];
...
function summarizeSelected() {
  ...
  } else if (['thread1', 'thread2', 'thread3', 'thread4', 'thread6', 'thread7'].includes(scope)) {
    summaryHTML = window[`summ${scope.charAt(0).toUpperCase() + scope.slice(1)}ByComment`];

  } else if (scope === 'thread5') {
    if (format === 'single') {
      summaryHTML = summThread5Single;
    } else if (format === 'single-username') {
      summaryHTML = summThread5Username;
    } else if (format === 'per-user') {
      summaryHTML = summThread5ByUser;
    } else if (format === 'per-comment') {
      summaryHTML = summThread5ByComment;
    }

  } else if (scope === 'thread8') {
    if (format === 'single') {
      summaryHTML = summThread8Single;
    } else if (format === 'single-username') {
      summaryHTML = summThread8Username;
    } else if (format === 'per-user') {
      summaryHTML = summThread8ByUser;
    } else if (format === 'per-comment') {
      summaryHTML = summThread8ByComment;
    }
  }
